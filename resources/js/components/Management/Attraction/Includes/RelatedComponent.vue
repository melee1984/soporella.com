...
<script>
    export default {
      ...
      methods: {
        fetchData: function() {
          var self = this;
          axios.get('/api/management/'+ self.attraction.id +'/related?api_token='+api_token).then(function (response) {
        ...
        updateMapping: function(obj) {
          var self = this;
          axios.post('/api/management/related/'+self.attraction.id+'/add/'+obj.id+'/submit?api_token='+api_token).then(function (response) {
